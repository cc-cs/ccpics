...
def process_submissions():
	...
	for submission_id in submission_ids:
		...
		result = {'user-id': submission['user-id'],
				  ...
				  'compile_message': test_results['compile_message']
...
def run(filename, test_cases, time_out, settings, source_json):
	...
	if test_cases:
		for test in test_cases:
			if 'compile' in settings:
				...
				results['compile_message'] = str(p.stderr.read())
			if 'compile' in settings and not os.path.exists(settings['CompiledFile']):
	...
	# run no input scenario
		...
		if 'compile' in settings:
				...
				p = subprocess.Popen(settings['compile'], shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
				results['compile_message'] = str(p.stderr.read())
		if 'compile' in settings and not os.path.exists(filename + '.exe'):
			results['ERROR'] = COMPILATION_ERROR
			#outputs.append(("CE", "TO", ''))
		else:
			p = subprocess.Popen(settings['run'], shell=True,
...
def load_code(source_json, test_cases, time_out):
	...
	file_create(source_json)
	if language == 'c++':
		z = run_cpp(filename, test_cases, time_out, source_json)
		os.system('rm -rf temp')
		# os.system('rm %s' % (filename + '.cpp'))
		# os.system('rm %s' % (filename + '.exe'))
		return z
	elif language == 'java':
		z = run_java(filename, test_cases, time_out, source_json)
		os.system('rm -rf temp')
...
def grade_submission(submission, solution, question):
	...
	result['compile_message'] = submission_results['compile_message']
	grades = []
	...
	if 'ERROR' in submission_results:
		...
		return result
	i = 0
	for test_case in question['test-cases']:
		result['test-cases'][test_case] = {}
		result['test-cases'][test_case]['time_taken'] = submission_results['times'][i]
	...
	if not question['test-cases']:
		result['test-cases']['__default__']['time_taken'] = submission_results['times'][0]
